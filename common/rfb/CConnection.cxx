...
void CConnection::sendKeyRelease(int systemKeyCode)
{
  ...
  static int just_unexpected_released;
  ...
  if (iter == downKeys.end()) {
    ...
    if ((systemKeyCode == 50 && just_unexpected_released == 64) ||
        (systemKeyCode == 64 && just_unexpected_released == 50)) {
      vlog.debug("Unexpected? Sending shift-alt!");
      sendKeyPress(0xfffe, 0x38, XK_ISO_Next_Group);
      sendKeyRelease(0xfffe);
      just_unexpected_released = 0;
    }
    else
      just_unexpected_released = systemKeyCode;
    return;
  }
